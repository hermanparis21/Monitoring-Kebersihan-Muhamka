...
# --- DASHBOARD PENGAWAS ---
    ...
    total = sum(len(v) for v in t_today.values())
    done = len(logs[logs['tanggal'] == datetime.now(jakarta_tz).strftime("%Y-%m-%d")]) if not logs.empty else 0
    
    st.metric("Progress Hari Ini", f"{done} / {total} Tugas")
    st.progress(min(done/total, 1.0) if total > 0 else 0)

    t1, t2, t3 = st.tabs(["📊 Histori", "🛠️ Laporan Perbaikan", "📣 Komplain"])
    
    with t1:
        f_tgl = st.date_input("Pilih Tanggal")
        if not logs.empty:
            view = logs[logs['tanggal'] == str(f_tgl)]
